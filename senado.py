...
PARSER_V         = 1     # subilo si cambia el parseo: fuerza releer todo
...
def leer_informe(html):
    ...
    for tabla in tablas_de(html):
        ...
        for fila in tabla[1:]:
            ...
            activo = re.sub(r"\s*\bRate/Coupon:.*$", "", g("activo"), flags=re.I)
            ticker = g("ticker")
            if ticker in ("--", "-", "N/A"):
                ticker = ""
...
def main():
    ...
    print("Abriendo sesión en efd.senate.gov…")
    token = abrir_sesion()
