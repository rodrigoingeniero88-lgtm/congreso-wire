...
<div class="tools">
  ...
  <div class="chips" id="chips">
    ...
    <button class="chip" data-f="house" aria-pressed="false">Cámara</button>
    <button class="chip" data-f="senate" aria-pressed="false">Senado</button>
  </div>
  <div class="chips">
...
async function cargar(){
  try{
    ...
    $('.sub').textContent = 'Cámara y Senado · actualizado ' +
...
function pasa(f){
  ...
  const cam = f.camara || 'Cámara';
  if(FILTRO === 'house'  && cam !== 'Cámara') return null;
  if(FILTRO === 'senate' && cam !== 'Senado') return null;

  let ops = f.operaciones || [];

  // Sin operaciones legibles (papel escaneado, o PDF que no se pudo leer).
  // Se muestra igual, con el enlace al original.
  if(!ops.length){
    if(SOLO_ACC) return null;
    if(FILTRO !== 'all' && FILTRO !== 'house' && FILTRO !== 'senate') return null;
    if(q && !f.nombre.toLowerCase().includes(q)) return null;
  ...
  vis.slice(0, TOPE).forEach(f => {
    ...
    const ops = f.operaciones.map(o => {
      ...
      return '<div class="op">'+
        ...
        (o.cruce ? '<div class="cruce">Integra el comité de '+
           esc(o.cruce.comite)+', que regula '+esc(o.cruce.sector)+'</div>' : '')+
    ...
    el.innerHTML =
      ...
      '<div class="when"><span class="cam '+
        ((f.camara === 'Senado') ? 'sen">Senado' : 'rep">Cámara')+'</span> '+
        esc(f.distrito || '')+' · declarado '+
        bonita(f.fecha_aviso)+' · '+f.operaciones.length+' op.</div>'+
      '<a class="pdf" href="'+f.pdf+'" target="_blank" rel="noopener">'+
        (f.camara === 'Senado' ? 'Ver el PTR en el eFD del Senado' : 'Ver el PTR original')+
      '</a>'+
      (f.operaciones.length ? ops :
        '<div class="vacia">Sin operaciones legibles — presentación en papel o '+
        'PDF escaneado. El enlace lleva al original.</div>');
    frag.appendChild(el);
  });
...
function esc(s){
  // Algunos PDF del Clerk dejan bytes nulos pegados al nombre del activo.
  return String(s ?? '').replace(/[\x00-\x1f]+/g, ' ').replace(/[&<>"]/g,
    c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
}
...
# El navegador no puede leer www.sec.gov/Archives (no habilita CORS), así que
# los Formularios 4 se leen acá y se publican ya masticados.

CIKS_SEC = [
    ("0001849635", "Trump Media (DJT)"),
    ("0000947033", "Donald J. Trump"),
    ("0002016181", "Donald J. Trump Jr."),
]

COD_SEC = {
    "P": ("compró en mercado abierto", True,  "buy"),
    "S": ("vendió en mercado abierto", True,  "sell"),
    "A": ("recibió del emisor",        False, "buy"),
    "D": ("devolvió al emisor",        False, "sell"),
    "F": ("entregó para cubrir impuestos", False, "sell"),
    "M": ("ejerció una opción",        False, "buy"),
    "G": ("donó",                      False, "sell"),
    "C": ("convirtió",                 False, "buy"),
    "X": ("ejerció un warrant",        False, "buy"),
}


def _txt(nodo, etiqueta):
    if nodo is None:
        return ""
    e = nodo.find(f".//{etiqueta}")
    return (e.text or "").strip() if e is not None and e.text else ""


def leer_form4(cik, acc_plano, doc):
    """Baja y parsea el XML de un Formulario 3/4/5."""
    base = f"https://www.sec.gov/Archives/edgar/data/{cik}/{acc_plano}/"
    rutas = []
    if doc and doc.lower().endswith(".xml"):
        rutas.append(base + re.sub(r"^xsl[^/]*/", "", doc, flags=re.I))
    rutas.append(base + "primary_doc.xml")

    crudo, motivo = None, ""
    for u in rutas:
        try:
            r = sesion.get(u, timeout=30, headers={
                "User-Agent": UA,
            ...
            motivo = str(e)[:60]
    if crudo is None:
        leer_form4.fallos.append(motivo or "sin respuesta")
        return None

    try:
        x = ET.fromstring(crudo)
    except ET.ParseError:
        return None

    rel = x.find(".//reportingOwnerRelationship")
    rol = []
    if rel is not None:
        si = lambda t: _txt(rel, t).lower() in ("1", "true")
        if si("isDirector"):
            rol.append("Director")
        if si("isOfficer"):
            rol.append(_txt(rel, "officerTitle") or "Directivo")
        if si("isTenPercentOwner"):
            rol.append("Más del 10%")

    txs = []
    for t in list(x.iter("nonDerivativeTransaction")) + list(x.iter("derivativeTransaction")):
        txs.append({
            "code":   _txt(t, "transactionCode"),
            "acc":    _txt(t.find(".//transactionShares"), "value"),
            "precio": _txt(t.find(".//transactionPricePerShare"), "value"),
            "queda":  _txt(t.find(".//sharesOwnedFollowingTransaction"), "value"),
        })

    return {
        "owner":    _txt(x.find(".//reportingOwnerId"), "rptOwnerName"),
        "rol":      " · ".join(rol),
        "simbolo":  _txt(x, "issuerTradingSymbol"),
        "emisor":   _txt(x, "issuerName"),
        "txs":      txs,
        "decision": any(COD_SEC.get(t["code"], (None, False))[1] for t in txs),
    }
...
def djt():
    """Publica docs/djt.json con las presentaciones y su detalle ya leído."""
    print("\nLeyendo EDGAR (DJT)…")
    vistos, filas = set(), []

    for cik, fuente in CIKS_SEC:
        try:
            j = sesion.get(f"https://data.sec.gov/submissions/CIK{cik}.json",
                           timeout=45).json()
            R = j["filings"]["recent"]
        except Exception as e:
            print(f"  ! {fuente}: {e}")
        ...
        for i, acc in enumerate(R["accessionNumber"][:120]):
            if acc in vistos:
                continue
            vistos.add(acc)
            filas.append({
                "id": acc, "cik": str(int(cik)), "fuente": fuente,
                "form": R["form"][i], "acc": acc,
                "accPlain": acc.replace("-", ""),
                "doc": R["primaryDocument"][i],
                "periodo": R["reportDate"][i] or "",
                "t": R["acceptanceDateTime"][i] or (R["filingDate"][i] + "T00:00:00"),
            })

    filas.sort(key=lambda f: f["t"], reverse=True)
    filas = filas[:150]

    leidos = 0
    for f in filas:
        if not re.match(r"^[345](/A)?$", f["form"]):
            continue
        d = leer_form4(f["cik"], f["accPlain"], f["doc"])
        if d:
            f["detalle"] = d
            leidos += 1
        time.sleep(0.2)

    destino = os.path.join(RAIZ, "docs", "djt.json")
    os.makedirs(os.path.dirname(destino), exist_ok=True)
    with open(destino, "w", encoding="utf-8") as fh:
        json.dump({"actualizado": dt.datetime.now(dt.timezone.utc)
                                    .strftime("%Y-%m-%dT%H:%M:%SZ"),
                   "datos": filas}, fh, ensure_ascii=False, separators=(",", ":"))
    print(f"  {len(filas)} presentaciones, {leidos} formularios leídos")
    if leer_form4.fallos:
        from collections import Counter
        for motivo, n in Counter(leer_form4.fallos).most_common(3):
            print(f"  ! {n} fallos: {motivo}")
...
def main():
    ...
    try:
        ...
        print(f"! DJT falló, se continúa: {e}")
