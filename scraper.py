...
PARSER_V        = 2     # subilo si cambia el parseo: fuerza releer los PDF
...
def limpiar_activo(texto):
    ...
    # Restos de la fila anterior: importes sueltos, "Over $X", ids de cuenta,
    # la N/Y de "Cap. Gains". No vienen siempre en el mismo orden, así que se
    # repasan en bucle hasta que ninguno aplique.
    SOBRAS = (
        r"^\s*(?:Spouse/DC|Joint)\b",
        r"^\s*Over\s*\$[\d,]+",
        r"^\s*\$[\d,]+(?:\s*-\s*\$[\d,]+)?",
        r"^\s*[NY]\b",
        r"^\s*\d{6,}\b",
...
def leer_pdf(contenido):
    ...
    crudo = re.sub(r"[\x00-\x08\x0b\x0c\x0e-\x1f]", " ", crudo)

    # El PDF intercala campos de metadatos entre las filas. Sus etiquetas salen
    # como glifos rotos ("F S :", "S O :", "L :", "D :"). Si aplanamos el texto
    # sin sacarlas, se pegan al nombre del activo siguiente.
    utiles = [ln for ln in crudo.splitlines() if not META.match(ln)]
    plano = re.sub(r"\s+", " ", " ".join(utiles))
    plano = re.sub(r"\b(?:F\s*S|S\s*O|L|D|C)\s*:\s*", " ", plano)
