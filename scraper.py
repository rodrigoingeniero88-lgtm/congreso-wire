...
PARSER_V        = 3     # subilo si cambia el parseo: fuerza releer los PDF
        ...
        with pdf:
            for pagina in pdf.pages:
                ...
                for ws in renglones:
                    ...
                    extra = c.get("Asset", "")
                    n = ACCIONES.search(extra)
                    if n:
                        abierta["acciones"] = n.group(1)

                    if META.match(extra) or ACCIONES.search(extra) or ":" in extra[:16]:
                        pass                      # metadatos, no forman parte del nombre
                    elif extra:
                        abierta["activo"] += " " + extra

                    if c.get("Amount"):           # monto partido en dos renglones
                        abierta["monto"] += " " + c["Amount"]
